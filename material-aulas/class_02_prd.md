...
#### Especificação OpenAPI - Clientes
...
* **Critérios de Aceite**:
  ...
  * A aplicação proverá caminho receptivo onde novas intenções constarão itens contábeis mandatórios (como o valor principal, a identificação do dono - `clienteId`, a **quantidade de parcelas (meses)** e o sistema a ser usado `SAC/PRICE`). Sem a definição do prazo (parcelas), o cálculo não pode ser processado.
...
#### Especificação OpenAPI - Empréstimos
...
## 4. Guia de Referência: Cálculos de Amortização

Para auxiliar na implementação da lógica de negócio, utilize as fórmulas abaixo como base para os sistemas SAC e PRICE.

### Sistema de Amortização Constante (SAC)

No sistema SAC, o valor da amortização é fixo em todas as parcelas. Os juros e a prestação diminuem ao longo do tempo.

1. **Amortização (A)**: `Valor Total / Quantidade de Parcelas`
2. **Juros da Parcela (J_n)**: `Saldo Devedor Anterior * Taxa de Juros Mensal`
3. **Prestação (P_n)**: `Amortização + Juros da Parcela`
4. **Saldo Devedor (SD_n)**: `Saldo Devedor Anterior - Amortização`

### Sistema Francês (PRICE)

No sistema PRICE, o valor da prestação (parcela) é constante durante todo o contrato. A proporção entre juros e amortização varia em cada mês.

1. **Prestação Fixa (P)**: `Valor Total * [((1 + i)^n * i) / ((1 + i)^n - 1)]`
    * *Onde: i = taxa de juros (ex: 0.02 para 2%), n = número de parcelas.*
2. **Juros da Parcela (J_n)**: `Saldo Devedor Anterior * Taxa de Juros Mensal`
3. **Amortização (A_n)**: `Prestação - Juros da Parcela`
4. **Saldo Devedor (SD_n)**: `Saldo Devedor Anterior - Amortização`
